...
- Refund         → 3 pohyby, ZRKADLOVÝ obraz Order Payment: smer (receipt/expense)
                   je obrátený pri všetkých troch pohyboch, sumy sú kladné. Pohoda
                   totiž priceSum interpretuje vždy ako kladnú sumu — smer určuje
                   výhradne bnk:bankType.
...
def generate_xml(
    ...
    for _, row in df_all.iterrows():
        ...
        # ── Refund ────────────────────────────────────────────────────────
            ...
            # určuje výhradne bankType. Refund je zrkadlový obraz Order Payment:
            # smer sa OBRACIA pri všetkých troch pohyboch, sumy sú kladné (absolútne).
            amt_receipt = _round2(abs(float(row["Total product charges"])))
            amt_fee     = _round2(abs(float(row["Amazon fees"])))
            amt_net     = _round2(abs(float(row["Total (USD)"])))

            # Pohyb 1: výdaj hrubej sumy (obrátené oproti Order Payment - refund
            # znižuje pôvodne vyrovnanú pohľadávku)
            mv = _pad(movement, 4)
            items_xml.append(_build_item(
                pack_id, idx, "expense", statement_str, mv, date_str,
            ...
            # Pohyb 2: príjem poplatku (obrátené oproti Order Payment)
            mv = _pad(movement, 4)
            items_xml.append(_build_item(
                pack_id, idx, "receipt", statement_str, mv, date_str,
                ACCOUNTING_FEE, TEXT_FEE, None, amt_fee, rate,
            ...
            # Pohyb 3: príjem netto sumy (obrátené oproti Order Payment)
            mv = _pad(movement, 4)
            items_xml.append(_build_item(
                pack_id, idx, "receipt", statement_str, mv, date_str,
                ACCOUNTING_NET, TEXT_NET, None, amt_net, rate,
