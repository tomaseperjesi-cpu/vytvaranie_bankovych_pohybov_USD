...
- Refund         → 3 pohyby, ZRKADLOVÝ obraz Order Payment: smer (receipt/expense)
                   je obrátený, sumy sú kladné. Pohoda totiž priceSum interpretuje
                   vždy ako kladnú sumu — smer určuje výhradne bnk:bankType, takže
                   negatívna priceSum pri bankType=receipt by sa zaúčtovala ako
                   kladný príjem (rovnako ako pri bežnej objednávke).
...
def generate_xml(
    ...
    for _, row in df_all.iterrows():
        ...
        # ── Refund ────────────────────────────────────────────────────────
            ...
            # Pohoda interpretuje priceSum vždy ako kladnú sumu — smer (Príjem/Výdaj)
            # určuje výhradne bankType. Refund je preto zrkadlový obraz Order Payment:
            # smery (receipt/expense) sa OBRÁTIA, sumy sú kladné (absolútne hodnoty).
            amt_receipt = _round2(abs(float(row["Total product charges"])))
            amt_fee     = _round2(abs(float(row["Amazon fees"])))
            amt_net     = _round2(abs(float(row["Total (USD)"])))

            # Pohyb 1: výdaj hrubej sumy (peniaze odchádzajú späť zákazníkovi)
            mv = _pad(movement, 4)
            items_xml.append(_build_item(
                pack_id, idx, "expense", statement_str, mv, date_str,
            ...
            # Pohyb 2: príjem poplatku (Amazon vracia časť provízie späť)
            mv = _pad(movement, 4)
            items_xml.append(_build_item(
                pack_id, idx, "receipt", statement_str, mv, date_str,
                ACCOUNTING_FEE, TEXT_FEE, None, amt_fee, rate,
            ...
            # Pohyb 3: príjem netto vyrovnania
            mv = _pad(movement, 4)
            items_xml.append(_build_item(
                pack_id, idx, "receipt", statement_str, mv, date_str,
                ACCOUNTING_NET, TEXT_NET, None, amt_net, rate,
