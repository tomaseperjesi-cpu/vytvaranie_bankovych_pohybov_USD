...
def parse_invoice_lookup(file_obj) -> dict[str, InvoiceInfo]:
    ...
    číslo faktúry (typ:numberRequested, napr. '2025VFB7') -> InvoiceInfo
    (skutočný inv:symVar z Pohody + raw XML blok adresy zákazníka),
    ktorá sa použije ako autoritatívny zdroj variabilného symbolu."""
...
def _build_item(
    ...
    sympar_line = f"\n\t\t\t\t<bnk:symPar>{sym_par if sym_par else sym_var}</bnk:symPar>" if (sym_par or sym_var) else ""
...
def generate_xml(
    ...
    if not invoice_lookup:
        raise ProcessingError(
            ...
            "výhradne z neho, takže je tento súbor povinný."
        )
    ...
    for _, row in df_orders.iterrows():
        ...
        items_xml.append(
            _build_item(
                ...
                amt_receipt, rate,
                partner_block=info.partner_block,
                sym_par=sym_var,
            )
        )
        movement += 1
    ...
    if not items_xml:
        raise ProcessingError(
            "Žiadna objednávka sa nenapárovala na faktúru z Pohody - skontroluj, "
            "či nahraný XML export faktúr obsahuje aj tieto objednávky."
        )
    ...
    return RunResult(
        xml_bytes=xml_bytes,
        filename=filename,
        order_count=len(df_orders) - len(unmatched_fakturas),
