...
REQUIRED_COLUMNS_BASE = ["Transaction type"]


@dataclass(frozen=True)
class CurrencyConfig:
    currency: str          # ISO kód: USD, GBP, EUR
    account_ids: str       # Pohoda účet: USDA, GBPA, EURA
    is_home_currency: bool # True pre EUR — sumy sú už v domácej mene, bez foreignCurrency bloku


CURRENCY_CONFIGS: dict[str, "CurrencyConfig"] = {
    "USD": CurrencyConfig(currency="USD", account_ids="USDA", is_home_currency=False),
    "GBP": CurrencyConfig(currency="GBP", account_ids="GBPA", is_home_currency=False),
    "EUR": CurrencyConfig(currency="EUR", account_ids="EURA", is_home_currency=True),
}
...
def _compute_gross(row) -> Decimal:
    """Vypočíta hrubú sumu pohybu 1 (príjem/vrátenie tovaru):
       Total product charges + Total promotional rebates + Other.

    Stĺpce 'Total promotional rebates' a 'Other' sú nepovinné — ak
    v súbore chýbajú alebo sú NaN, použije sa 0. Tým je funkcia
    spätne kompatibilná s USD exportom, ktorý tieto stĺpce nemá.
    Sumy sa spočítajú pred zaokrúhlením, aby sa predišlo kumulácii
    chýb zaokrúhľovania."""
    gross = float(row["Total product charges"])
    for col in ("Total promotional rebates", "Other"):
        if col in row.index:
            val = row.get(col)
            if pd.notna(val):
                gross += float(val)
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Premenuje variantné stĺpce rôznych Amazon exportov na štandardné interné názvy.

    Podporované varianty:
      Total (GBP) / Total (EUR)  → Total (USD)  [neutrálny interný názov pre celkovú sumu]
      Faktúra/doklad              → Faktura
    """
    rename = {}
    # Total column: Total (GBP), Total (EUR) → interný štandard Total (USD)
    for col in df.columns:
        if (
            col.startswith("Total (")
            and col.endswith(")")
            and col != "Total (USD)"
            and col != "Total product charges"
        ):
            rename[col] = "Total (USD)"
            break
    # Faktura column variants
    for variant in ("Faktúra/doklad", "Faktura/doklad", "Faktura/Doklad"):
        if variant in df.columns:
            rename[variant] = "Faktura"
            break
...
def load_orders(file_or_path):
    ...
    # Normalizuj variantné stĺpce (napr. Total (GBP) → Total (USD), Faktúra/doklad → Faktura)
    ...
    if "Total (USD)" not in df.columns:
        raise ProcessingError(
            "V súbore chýba stĺpec s celkovou sumou — očakáva sa 'Total (USD)', "
            "'Total (GBP)' alebo 'Total (EUR)'."
        )
...
def _build_item(
    ...
    cfg: CurrencyConfig = CURRENCY_CONFIGS["USD"],
) -> str:
    number = f"{cfg.account_ids}{statement_number}{movement_number}"

    if cfg.is_home_currency:
        # EUR: sumy sú priamo v domácej mene, žiadna konverzia ani foreignCurrency blok
        home_amount = foreign_amount
        foreign_currency_xml = ""
    else:
        # USD / GBP: konvertuj na EUR, pridaj foreignCurrency blok
        ...
        foreign_currency_xml = f"""
\t\t\t\t<bnk:foreignCurrency>
\t\t\t\t\t<typ:currency>
\t\t\t\t\t\t<typ:ids>{cfg.currency}</typ:ids>
\t\t\t\t\t</typ:currency>
...
\t\t\t\t<bnk:account>
\t\t\t\t\t<typ:ids>{cfg.account_ids}</typ:ids>
\t\t\t\t</bnk:account>
...
\t\t\t\t\t\t<typ:priceRound>0</typ:priceRound>
\t\t\t\t\t</typ:round>
\t\t\t\t</bnk:homeCurrency>{foreign_currency_xml}
\t\t\t</bnk:bankSummary>
\t\t</bnk:bank>
\t</dat:dataPackItem>"""
...
def generate_xml(
    ...
    cfg = CURRENCY_CONFIGS.get(currency, CURRENCY_CONFIGS["USD"])
    ...
    pack_id = f"{cfg.currency}{date_min.strftime('%Y%m%d')}-{date_max.strftime('%Y%m%d')}"
    ...
    for _, row in df_all.iterrows():
        ...
        if tx_type == "order":
            ...
            amt_receipt = _compute_gross(row)
        ...
        # ── Refund ────────────────────────────────────────────────────────
            ...
            amt_receipt = _compute_gross(row)  # záporné as-is pre refund
            amt_fee     = _round2(-abs(float(row["Amazon fees"])))
