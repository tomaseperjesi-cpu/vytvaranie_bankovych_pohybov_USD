...
currency = st.radio(
    ...
    options=["USD", "GBP", "EUR"],
    horizontal=True,
    help="USD = amazon.de (USD účet USDA) · GBP = amazon.co.uk (GBPA) · EUR = amazon.de (EUR účet EURA)",
)
...
st.write(
    ...
    "- **Refund** → 3 pohyby so zápornými hodnotami (vrátane poplatku)\n\n"
...
**Refund** (3 pohyby):
- Rovnaká štruktúra ako Order Payment, hodnoty **záporné** (všetky tri pohyby).
- `Total product charges` a `Total (USD)` sú v zdrojových dátach záporné (as-is).
- `Amazon fees` je v zdrojových dátach kladné, preto sa explicitne zápornuje.
