...
**Refund** (3 pohyby — ZRKADLOVÝ obraz Order Payment):
- Pohoda interpretuje `priceSum` vždy ako kladnú sumu — smer (Príjem/Výdaj) určuje výhradne `bankType`.
  Preto sa pri Refund smery **obrátia** oproti Order Payment, sumy sú kladné (absolútne):
  1. **Výdaj** — suma = `abs(Total product charges)`, predkontácia `úh.VFA USD`, text "Refund FV č. {číslo faktúry}".
  2. **Príjem** — suma = `abs(Amazon fees)`, predkontácia `Uhr.OZ-USD`, text "POPLATOK ZA TRANSAKCIU".
  3. **Príjem** — suma = `abs(Total (USD))`, predkontácia `Amaz.OZ-USD`, text "NETTO čIASTKKA FKSTURY".
- Bez symVar, ak dobropis nie je v XML faktúr (číslo refundu typu D004 sa zvyčajne nenapáruje).
